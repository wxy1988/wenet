...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        if self.ctc_weight != 1.0:
            l_loss_att, r_loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_mask,
                                                                  text, text_lengths)
            # combine both left to right and right to left loss (optional)
            if r_loss_att is not None:
                loss_att = l_loss_att * 0.7 + 0.3 * r_loss_att
            else:
                loss_att = l_loss_att
        else:
            loss_att = None
    ...
    def _calc_att_loss(
        ...
        ys_in_pad, ys_out_pad = add_sos_eos(ys_pad, self.sos, self.eos,
                                            self.ignore_id)
        r_ys_in_pad = None
        if self.decoder.r_num_blocks > 0:
            r_ys_in_pad, r_ys_out_pad = add_eos_sos(ys_pad, self.sos, self.eos,
                                                    self.ignore_id)
        ys_in_lens = ys_pad_lens + 1

        # 1. Forward decoder
        decoder_out, r_decoder_out, _ = self.decoder(encoder_out, encoder_mask, ys_in_pad,
                                                     ys_in_lens, r_ys_in_pad)

        # 2. Compute attention loss
        l_loss_att = self.criterion_att(decoder_out, ys_out_pad)
        r_loss_att = None
        # 3. Compute attention loss (optional)
        if r_decoder_out is not None:            
    ...
    def attention_rescoring(
        ...
        l_hyps_pad, _ = add_sos_eos(hyps_pad, self.sos, self.eos,
                                    self.ignore_id)
        r_hyps_pad, _ = add_eos_sos(hyps_pad, self.sos, self.eos,
                                    self.ignore_id)
        ...
        for i, hyp in enumerate(hyps):
            ...
            t = 0
            for j, w in enumerate(r_hyps_pad[i]):
                if w == self.eos:
                    t = t + 1
                else:
                    break
            # add right to left score
            for j, w in enumerate(hyp[0]):
                r_score += r_decoder_out[i][j + 1 + t][w]
    ...
    def forward_attention_decoder(
        ...
        decoder_out, _, _ = self.decoder(
            encoder_out, encoder_mask, hyps, hyps,
